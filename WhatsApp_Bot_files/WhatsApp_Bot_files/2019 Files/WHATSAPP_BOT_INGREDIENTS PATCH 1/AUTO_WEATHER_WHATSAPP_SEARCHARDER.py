...
receiverlist = ["ERU, MOC, Zone, BM", "Team LMZ", "Russ" ]
...
for receiver in receiverlist:
    ...
    pyautogui.click(x=image_x, y=image_y, clicks=3, button='left')
    time.sleep(3)
    pyautogui.typewrite(receiver, interval=0.001)
    pyautogui.typewrite('\n', interval=0.001)
    print("Recipient " + receiver + " found.")
    time.sleep(10)

    #Check time before sending message
    if receiver == "Team LMZ" and (datetime.datetime.now().strftime('%H%M')) > '0730' and (datetime.datetime.now().strftime('%H%M')) < '2130' and (nz != [] or ez != [] or cl != [] or wz != []):
        recipient = "Team LMZ"    
        send_msg(recipient, rainzone_msg)
        
    elif receiver == "Team LMZ" and (datetime.datetime.now().strftime('%H%M')) > '0030' and (datetime.datetime.now().strftime('%H%M')) < '0430'  and (nz != [] or ez != [] or cl != [] or wz != []):
        recipient = "Good Evening LMZ"
        send_msg(recipient, rainzone_msg)

    #Check time before sending message
    if receiver == "ERU, MOC, Zone, BM" and (datetime.datetime.now().strftime('%H%M')) > '0730' and (datetime.datetime.now().strftime('%H%M')) < '2130'  and (nz != [] or ez != [] or cl != [] or wz != []):
        recipient = "Hello ERU"
        send_msg(recipient, rainzone_msg)
        
    elif receiver == "ERU, MOC, Zone, BM" and (datetime.datetime.now().strftime('%H%M')) > '0030' and (datetime.datetime.now().strftime('%H%M')) < '0430'  and (nz != [] or ez != [] or cl != [] or wz != []):
    ...
    if receiver == "Russ" and (datetime.datetime.now().strftime('%H%M')) > '0730' and (datetime.datetime.now().strftime('%H%M')) < '2130'  and (nz != [] or ez != [] or cl != [] or wz != []):
        recipient = "Hello Russ" #"Hello ERU"
        send_msg(recipient, rainzone_msg)

        time.sleep(10800)#last person triggers delay
        
    elif receiver == "Russ" and (datetime.datetime.now().strftime('%H%M')) > '0030' and (datetime.datetime.now().strftime('%H%M')) < '0430'  and (nz != [] or ez != [] or cl != [] or wz != []):
        recipient = "Hi Russ!" #"Attention all Maintenance Teams"
        send_msg(recipient, rainzone_msg)
